...
def asyncSendFireAlarm(image_file, conf):
    ...
    if last_call_time is None or current_time - last_call_time >= kMinAlarmEventIntervalInS:
        try:
            response = requests.post(kServerApi, files={'image':open(image_file, 'rb')}, data={'conf':conf, 'timestamp':current_time})
