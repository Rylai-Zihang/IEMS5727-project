...
def upload_temperature():
    ...
    detected_at = request.form.get("timestamp", "")
    global db
    db.insert_temperature_data(
        device, round(float(temperature_value), 2), int(detected_at)
    )
...
def query_analyse_data():
    global db
    log_data = db.get_recent_fires_with_images(20)
    ...
    risk_data = analysis_risk(
        db.get_recent_device_tempeature_data(2), db.get_recent_fires_by_device(2)
    )
